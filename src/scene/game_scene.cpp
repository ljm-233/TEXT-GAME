...
GameScene::GameScene(std::shared_ptr<Background> background, const sf::Font& font,
    ...
    if (!loadLevel(levelIndex_)) {
        ...
        NotificationSystem::instance().push("关卡加载失败", NotificationType::Error, 5.f);
    }
...
bool GameScene::loadLevel(int index) {
    std::string path = std::string(PROJECT_ROOT) + "/assets/levels/level" +
...
void GameScene::refreshHud() {
    ...
    std::string hud = "存档: " + save_.name + "   关卡: " + std::to_string(levelIndex_) +
...
void GameScene::refreshOverlayLayout(float winW, float winH) {
    ...
    overlayTitle_.setFillColor(state == GameWorld::State::LevelComplete
...
void GameScene::handleEvent(const sf::Event& event) {
    ...
    if (const auto* kp = event.getIf<sf::Event::KeyPressed>()) {
        if (kp->code == sf::Keyboard::Key::Escape) {
            paused_ = true;
            pauseMenu_ = std::make_unique<PauseMenu>(*font_, preferences_,
        ...
        if (kp->code == sf::Keyboard::Key::R) {
            world_->reset();
            lastState_ = GameWorld::State::Playing;
            NotificationSystem::instance().push("已重生", NotificationType::Info);
            return;
        }
    }
...
void GameScene::update(float dt) {
    ...
    if (world_->state() != GameWorld::State::Playing)
        return;
    ...
    if (cur != lastState_) {
        if (cur == GameWorld::State::LevelComplete) {
            ...
            NotificationSystem::instance().push("关卡完成！按 Enter 继续",
                                                NotificationType::Success, 5.f);
        } else if (cur == GameWorld::State::GameOver) {
            SoundManager::instance().playGameOver();
            NotificationSystem::instance().push("游戏失败，按 R 重试",
...
void GameScene::renderStateOverlay(sf::RenderTarget& rt, float winW, float winH) {
    refreshOverlayLayout(winW, winH); // 内部判断是否需要重算
