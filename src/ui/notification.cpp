...
void NotificationSystem::push(const std::string& text,
...
void NotificationSystem::update(float dt) {
    for (auto& e : queue_) e.elapsed += dt;
    while (!queue_.empty() &&
           queue_.front().elapsed >= queue_.front().duration) {
        queue_.pop_front();
    }
}

void NotificationSystem::render(sf::RenderTarget& target) {
    if (!enabled_ || !font_ || queue_.empty()) return;
    ...
    switch (pos_) {
        ...
        case NotificationPos::BottomLeft:  x = margin; break;
        case NotificationPos::TopRight:
        case NotificationPos::BottomRight: x = w - width - margin; break;
    }
    ...
    switch (pos_) {
        case NotificationPos::TopLeft:
        case NotificationPos::TopRight:    y = margin; break;
        case NotificationPos::BottomLeft:
        case NotificationPos::BottomRight: y = h - margin - totalH; break;
    }
    ...
    for (size_t i = startIdx; i < queue_.size(); ++i) {
        ...
        else if (t > 0.85f)    alpha = (1.f - t) / 0.15f;
        ...
        sf::Color bg;
        switch (e.type) {
            case NotificationType::Info:    bg = {50, 100, 170};  break;
            case NotificationType::Success: bg = {40, 130, 70};   break;
            case NotificationType::Warning: bg = {180, 130, 30};  break;
            case NotificationType::Error:   bg = {170, 50, 50};   break;
        }
        bg.a = a8;
        ...
        // 文字
        sf::Text text(*font_,
                      sf::String::fromUtf8(e.text.begin(), e.text.end()),
                      18);
