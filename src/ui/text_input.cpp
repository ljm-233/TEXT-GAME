...
void TextInput::setText(const std::string& text) {
    auto u32 = toU32(text);
    if (u32.size() > maxLength_) u32.resize(maxLength_);
    content_ = fromU32(u32);
    if (cursorPos_ > u32.size()) cursorPos_ = u32.size();
    refreshTexts();
    refreshCursor();
    notifyChanged();
}
...
void TextInput::notifyChanged() {
    if (onChanged_) onChanged_(content_);
}
...
void TextInput::refreshShape() {
    ...
    shape_.setOutlineColor(focused_ ? getTheme().buttonSelected
                                    : getTheme().outline);
}
    ...
    if (const auto* kp = event.getIf<sf::Event::KeyPressed>()) {
        ...
        switch (kp->code) {
            ...
            case sf::Keyboard::Key::Left:
                if (cursorPos_ > 0) { --cursorPos_; refreshCursor(); }
                blinkClock_.restart();
                break;

            case sf::Keyboard::Key::Right:
                if (cursorPos_ < u32.size()) { ++cursorPos_; refreshCursor(); }
                blinkClock_.restart();
                break;

            case sf::Keyboard::Key::Home:
                cursorPos_ = 0; refreshCursor();
                blinkClock_.restart();
                break;

            case sf::Keyboard::Key::End:
                cursorPos_ = u32.size(); refreshCursor();
                blinkClock_.restart();
                break;

            case sf::Keyboard::Key::Enter:
                if (onSubmit_) onSubmit_(content_);
                break;
